    ...
    while True:
        ...
        for event in pygame.event.get():
            if event.type == pygame.JOYBUTTONDOWN:
                ...
                if is_recording:
                    current_session_id_catalog = datetime.now().strftime("%Y-%m-%d")
                    current_session_id_image = datetime.now().strftime("%Y-%m-%d-%H%M%S")
                    filename = f"/home/donkey/projects/end2end-thesis/data/catalog_{current_session_id_catalog}.catalog"
                    current_catalog_file = open(filename, 'a')
                    record_index = 0
                    print("Recording Started")
                else:
                    if current_catalog_file is not None:
                        current_catalog_file.close()
                        current_catalog_file = None
        ...
        if is_recording and (current_time - last_record_time) >= 0.05:
            ...
            image_filename = f"/home/donkey/projects/end2end-thesis/data/images/{current_session_id_image}_{record_index}.jpg"
            image_name_root = image_filename.split('/')
            if frame is not None:
                cv2.imwrite(image_filename, frame)
            
            robot_data['index'] = record_index
            robot_data['session_id'] = current_session_id_image
            robot_data['timestamp_ms'] = int(time.time()) * 1000
            robot_data['cam/image_array'] = f"{image_name_root[-2]}/{image_name_root[-1]}"
            robot_data['angle'] = steering
            robot_data['user/mode'] = "user"
            robot_data['throttle'] = speed

            json_string = json.dumps(robot_data)
            current_catalog_file.write(json_string + "\n")
            current_catalog_file.flush()
