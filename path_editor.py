...
def pixel_to_cell(x, y):
    """Convert a pixel coordinate to a grid cell (col, row)."""
...
def mouse_callback(event, x, y, flags, param):
    """Left-click adds a cell; right-click removes the last added cell."""
...
def draw_frame(base_frame):
    ...
    for col, row in highlighted_cells:
        ...
        cv2.rectangle(frame, top_left, bottom_right, (0, 200, 0), -1)   # filled green
...
def save_path():
    """Save ordered waypoint centers to path.txt as x,y lines."""
...
def load_existing_path():
    """Pre-populate from an existing path.txt if present."""
            ...
            for line in f:
                ...
                if line:
                    ...
                    if cell not in cell_set:
                        highlighted_cells.append(cell)
                        cell_set.add(cell)
        print(f"[path_editor] Loaded {len(highlighted_cells)} existing waypoints from path.txt")
...
def main():
    ...
    if not ret:
        ...
        print("[path_editor] No camera found — using black canvas.")
    cap.release()
